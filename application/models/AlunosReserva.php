...
class Application_Model_AlunosReserva extends Zend_Db_Table_Abstract
{

    protected $_name = "alunoreservasite";
    ...
    public function buscarCpf($cpf = "", $email = "")
    {
        $select = $this->select()
                    ->setIntegrityCheck(false)
                    ->from(array('a' => 'alunoreservasite'))
                    ->where('cpf = ?', $cpf)
                    ->orWhere('email = ?', $email);
        return $this->fetchRow($select);
    }
